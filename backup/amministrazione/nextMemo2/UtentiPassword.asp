...
<% 
	...
	intestazione.sezione = "Gestione utenti area riservata - password"
	intestazione.scrivi_ridotta()
	
dim conn, rs, sql, saved
set conn = Server.CreateObject("ADODB.Connection")
conn.open Application("DATA_ConnectionString"),"",""
set rs = Server.CreateObject("ADODB.RecordSet")

if CInteger(request("ID")) = 0 then
	Session("ERRORE") = "Utente non Trovato"
%>
<div id="content_ridotto">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="caption"><%= Session("ERRORE") %></td>
...
sql = "SELECT * FROM tb_utenti INNER JOIN tb_Indirizzario ON tb_utenti.ut_nextCom_ID=tb_indirizzario.IDElencoIndirizzi " &_
	  " WHERE ut_ID=" & cIntero(request("ID"))
rs.open sql, conn, adOpenForwardOnly, adLockOptimistic, adCmdText
saved = false

if request("salva")<>"" then
	if request("tft_ut_password")="" OR  (not CheckChar(request("tft_ut_login"), LOGIN_VALID_CHARSET)) then
		Session("ERRORE") = "Password mancante o non valida! Utilizzare solo caratteri alfanumerici o &quot;_&quot;"
	elseif uCase(request("tft_ut_password")) <> uCase(request("conferma_password")) then
		Session("ERRORE") = "Errore nella conferma della password!"
	else
		'esegue la modifica
		rs("ut_password") = UCASE(request("tft_ut_password"))
		rs.Update
		saved = true
	end if
end if
...
<div id="content_ridotto">
	...
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		<caption>
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="caption">Utente: &quot;<%= ContactFullName(rs) %>&quot;</td>
					<td align="right" style="font-size: 1px;">
						<a class="button" href="javascript:window.close();">
							CHIUDI
						</a>
		...
		<% else %>
			...
			<tr>
				...
				<td class="content" style="width:70%;">
					<input type="password" class="text" name="tft_ut_password" value="<%= request("tft_ut_password") %>" maxlength="50" size="20">
				</td>
			</tr>
			<tr>
