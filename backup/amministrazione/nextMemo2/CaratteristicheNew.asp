...
dicitura.sezione = "Gestione caratteristiche - nuova"
...
<div id="content">
	...
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		<caption>Inserimento nuova caratteristica</caption>
		...
		<tr>
			...
			<td class="content"><input type="checkbox" class="checkbox" name="chk_ct_principale" <%= chk(request("chk_ct_principale")<>"") %>></td>
		</tr>
						...
						<% while not rsc.eof %>
							<tr>
								<td class="content_center">
									...
									<% else 
										value = instr(1, request("categorie_associate"), " " & rsc("catC_id") & " ", vbTextCompare)>0%>
								...
								<td class="content_center"><input <%= disable(not value) %> type="text" class="<%= IIF(not value, "text_disabled", "text") %>" size="2" name="rel_ordine_<%= rsc("catC_id") %>" value="<%= request("rel_ordine_" & rsc("catC_id")) %>"></td>
