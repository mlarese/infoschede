	...
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
			...
			lingua = Application("LINGUE")(i)%>
				...
				<% 	end if %>
					<td class="content" colspan="2">
						...
						<input type="text" class="text" name="tft_promo_nome_<%= lingua %>" value="<%= request("tft_promo_nome_"& lingua) %>" maxlength="255" style="width:90%;">
		...
		<%for i=lbound(Application("LINGUE")) to ubound(Application("LINGUE"))
			lingua = Application("LINGUE")(i)%>
				...
				<% 	end if %>
					<td class="content" colspan="2">
						<img src="../grafica/flag_<%= lingua %>.jpg">
						<input type="text" class="text" name="tft_promo_descrizione_<%= lingua %>" value="<%= request("tft_promo_descrizione_"& lingua) %>" maxlength="1000" style="width:90%;">
						<% 	if lingua = LINGUA_ITALIANO then response.write "(*)" end if %>
					</td>
				</tr>
			<%
		...
		<tr>
			...
			<td class="content" colspan="3">
				<input type="text" <%= disabled %> class="text" name="tfn_promo_valore" size="3" value=""> %
			</td>
		</tr>
		<tr>
			<td class="label">inizio validit&agrave;:</td>
			<td class="content" colspan="3">
				<% CALL WriteDataPicker_Input("form1", "tfd_promo_inizio_validita", "", "", "/", true, true, LINGUA_ITALIANO) %>
			</td>
		</tr>
		<tr>
			<td class="label">fine validit&agrave;:</td>
			<td class="content" colspan="3">
				<% CALL WriteDataPicker_Input("form1", "tfd_promo_fine_validita", "", "", "/", true, true, LINGUA_ITALIANO) %>
			</td>
		</tr>
