...
<!-- BLOCCO DI RICERCA -->
						...
						<tr>
							<td>
								<table cellspacing="1" cellpadding="0" class="tabella_madre">
									...
									<tr><th colspan="2" <%= Search_Bg("marchi_denominazione") %>>CLIENTE</th></tr>
									<tr>
			...
			<td valign="top">
				<% dim baseUrl 
				if cIntero(Session("B2B_PAGINA_OFFERTE_MARCHIO"))>0 then
					baseUrl = GetPageSiteUrl(conn, Session("B2B_PAGINA_OFFERTE_MARCHIO"), LINGUA_ITALIANO)
				else
					baseUrl = ""
				end if %>
				<!-- BLOCCO RISULTATI -->
				<table cellspacing="1" cellpadding="0" class="tabella_madre">
					...
					<% if not rs.eof then %>
						<tr>
							...
							<th class="center" style="width:6%;">GEN.</th>
							<th class="center" colspan="1" style="width:34%;">OPERAZIONI</th>
						</tr>
						...
						while not rs.eof and rs.AbsolutePage = Pager.PageNo%>
							<tr>
								...
								<td class="content_center">
									<% 	if CString(rs("mar_logo")) <> "" then %>
										<img style="width:100px;" src="http://<%= Application("IMAGE_SERVER") &"/"& Application("AZ_ID") &"/images/"& rs("mar_logo") %>" alt="logo" border="0">
									<% 	else %>
										&nbsp;
									<% 	end if %>
								</td>
								<td class="content_center">
									<input type="checkbox" disabled class="checkbox" <%= chk(rs("mar_generica")) %>>
								</td>
								<td class="content_center">
									<table>
										<tr>
							...
							<%
							if cString(baseUrl)<>"" then %>
									<tr>
										<td colspan="5" class="content_right" style="padding-bottom:15px;">
											<a class="button_l2" target="_blank" href="<%=baseUrl%>&MAR_ID=<%=rs("mar_id")%>">Elenco prodotti</a>
											&nbsp;
											<a class="button_l2" target="_blank" href="<%=baseUrl%>&MAR_ID=<%=rs("mar_id")%>&offerta=true">Elenco offerte</a>
											&nbsp;
											<a class="button_l2" target="_blank" href="<%=baseUrl%>&MAR_ID=<%=rs("mar_id")%>&bundle=true">Elenco bundle</a>
											&nbsp;
											<a class="button_l2" target="_blank" href="<%=baseUrl%>&MAR_ID=<%=rs("mar_id")%>&bundle=true&offerta=true">Elenco bundle in offerta</a>
										</td>
									</tr>
							<% end if
							rs.movenext
