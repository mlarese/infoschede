...
dicitura.sezione = "Gestione caratteristhce tecniche - elenco"
...
if session("ctech_categoria")<>"" then
	sql = sql & " AND ct_id IN (SELECT rct_ctech_id FROM gtb_tip_ctech WHERE rct_tipologia_id=" & session("ctech_categoria") & ") "
end if
...
sql = " SELECT *, (SELECT COUNT(*) FROM grel_art_ctech WHERE rel_ctech_id=ct_id) AS N_ART " + _
	  " FROM gtb_carattech " + _
	  " WHERE (1=1) " + sql + " ORDER BY ct_nome_it"
Session("B2B_CTECH_SQL") = sql
									...
									<table width="100%" border="0" cellspacing="1" cellpadding="0" align="left">
												...
												<table border="0" cellspacing="0" cellpadding="0" align="right">
													<tr>
														<td style="font-size: 1px;">
															...
															&nbsp;
															<% if rs("N_ART") > 0 then %>
																<a class="button_disabled" href="javascript:void(0);" title="Impossibile cancellare la caratteristica tecnica: sono presenti articoli associati">
																	CANCELLA
																</a>
															<% else %>
										...
										<tr>
											...
											<td class="content" style="max-height:200px; overflow-y:scroll; float:left; width:98%;">
												<% sql = "tip_L0.tip_id IN (SELECT rct_tipologia_id FROM gtb_tip_ctech WHERE rct_ctech_id=" & rs("ct_id") & ")"
