...
<%'--------------------------------------------------------
sezione_testata = "modifica commento" %>
...
sql = " SELECT * FROM v_indice INNER JOIN tb_comments ON v_indice.idx_id=tb_comments.com_idx_id " + _
	  " WHERE com_id =" & cIntero(request("ID"))
...
<div id="content_ridotto">
	<form action="" method="post" id="form1" name="form1">
		...
		<table cellspacing="1" cellpadding="0" class="tabella_madre">
			<caption>Modifica commento</caption>
			<tr><th colspan="3">DATI DEL COMMENTO</th></tr>
			<tr>
				<td class="label_no_width" colspan="2">contatto (*)</td>
				<td class="content">
					<% CALL WriteContactPicker_Input(conn, rsw, "", "", "form1", "tfn_com_contatto_id", rs("com_contatto_id"), "", false, false, false, "") %>		
				</td>
			</tr>
			<tr>
				...
				<td class="content">
					<%CALL dropDown(conn, "SELECT * FROM tb_comments_valutazioni ", _
							"val_id", "val_nome_it", "tfn_com_val_id", rs("com_val_id") , true, " style=""width=250""", LINGUA_ITALIANO)%>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="content">
					<textarea rows="5" style="width:95%;" name="tft_com_comment"><%= rs("com_comment") %></textarea>
					(*)
				</td>
			</tr>
			...
			<tr>
				...
				<td class="content">
						<input type="checkbox" class="checkbox" name="chk_com_validate" <% if rs("com_validate") then %> checked <% end if %> >
				</td>
			</tr>
