...
dicitura.sezione = "Analisi ricerche - Log parole chiave"
dicitura.puls_new = "INDIETRO A SITI;LOG COMPLETO"
dicitura.link_new = "Siti.asp;SitoAnalisiRicerche.asp"
...
sql = " SELECT lor_ricerca, COUNT(lor_ricerca) as lor_num_volte, AVG(lor_risultati_numero) as lor_media_risultati "+_
	  " FROM log_ricerche WHERE lor_web_id = "& session("AZ_ID") & sql & " GROUP BY lor_ricerca"
session("WEB_LOGRIC_SQL") = sql
						...
						<tr>
							<td>
								<table cellspacing="1" cellpadding="0" class="tabella_madre">
									...
									<tr><th colspan="2" <%= Search_Bg("logric_data_from;logric_data_to") %>>DATA DI ULTIMA MODIFICA</td></tr>
									...
									<tr><th colspan="2" <%= Search_Bg("logric_nome") %>>CHIAVE DI RICERCA</th></tr>
									<tr>
										<td class="content" colspan="2">
...
<!-- BLOCCO RISULTATI -->
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		<caption>Log parole chiave - Trovati n&ordm; <%= Pager.recordcount %> records in n&ordm; <%= Pager.PageCount %> pagine</caption>
		<% if not rs.eof then %>
			<tr>
				<th width="40%">CHIAVE DI RICERCA</th>
				<th>NUM. VOLTE UTILIZZATA</th>
				<th style="width:30%;">MEDIA RISULTATI OTTENUTI</th>
			</tr>
			<%rs.AbsolutePage = Pager.PageNo
				while not rs.eof and rs.AbsolutePage = Pager.PageNo%>
				<tr>
					<td class="content"><%= (rs("lor_ricerca")) %></td>
					<td class="content"><%= rs("lor_num_volte") %></td>
					<td class="content"><%= rs("lor_media_risultati") %></td>
