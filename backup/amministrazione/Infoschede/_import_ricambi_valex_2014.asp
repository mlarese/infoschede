...
ID_CATEGORIA = 1281 'Ricambi - Valex
ID_MARCA = 567 'Valex

%>
<div id="content">
<%
sql = " select [Codice], [Descrizione], [Listino 2014], [Qta min], [Cod Alternativo], [Note] from _2014_listino_valex "
...
<% while not s_rs.eof %>
	<table cellspacing="1" cellpadding="0" class="tabella_madre" style="border-bottom:1px">
		...
		if d_rs.eof then
			...
			d_rs("art_cod_alt") = Trim(s_rs("Cod Alternativo"))
			d_rs("art_nome_it") = Trim(s_rs("Descrizione"))
			d_rs("art_note") = Trim(s_rs("Note"))
			d_rs("art_disabilitato") = false
			d_rs("art_unico") = false

			d_rs("art_prezzo_base") = cReal(Trim(s_rs("Listino 2014")))
		...
		%>
			...
			<td width="20%" class="content"><%= cReal(Trim(s_rs("Listino 2014"))) %></td>
