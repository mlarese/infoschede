...
if request("goto")<>"" then
	CALL GotoRecord(conn, rsc, session("B2B_CTECH_SQL"), "ct_id", "CaratteristicheMod.asp")
end if
...
dicitura.sezione = "Gestione caratteristiche tecniche - modifica"
dicitura.puls_new = "INDIETRO"
dicitura.link_new = "Caratteristiche.asp"
dicitura.scrivi_con_sottosez() 


sql = "SELECT * FROM gtb_carattech WHERE ct_id="& cIntero(request("ID"))
	...
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		<caption>	
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="caption">Modifica dati della caratteristica tecnica</td>
					<td align="right" style="font-size: 1px;">
						<a class="button" href="?ID=<%= cIntero(request("ID")) %>&goto=PREVIOUS" title="caratteristica precedente" <%= ACTIVE_STATUS %>>
							&lt;&lt; PRECEDENTE
						</a>
						&nbsp;
						<a class="button" href="?ID=<%= cIntero(request("ID")) %>&goto=NEXT" title="caratteristica successiva" <%= ACTIVE_STATUS %>>
							SUCCESSIVA &gt;&gt;
						</a>
					</td>
				</tr>
		...
		<tr>
			<td colspan="2">
				<% dim value
				sql = ", (SELECT rct_ordine FROM gtb_tip_ctech WHERE rct_tipologia_id = TIP_L0.tip_id AND rct_ctech_id=" & rs("ct_id") & ") AS ORDINE " + _
						 ", (SELECT COUNT(*) FROM grel_art_ctech INNER JOIN gtb_articoli ON grel_art_ctech.rel_art_id = gtb_articoli.art_id " + _
						 "	 WHERE gtb_articoli.art_tipologia_id = TIP_L0.tip_id AND grel_art_ctech.rel_ctech_id=" & rs("ct_id") & ") AS N_ARTICOLI " + _
						 " FROM " %>
					...
					<% else %>
						...
						<% while not rsc.eof %>
							<tr>
								...
								<td class="content_center"><input <%= disable(not value) %> type="text" class="<%= IIF(not value, "text_disabled", "text") %>" size="2" name="rel_ordine_<%= rsc("tip_id") %>" value="<%= rsc("ORDINE") %>"></td>
