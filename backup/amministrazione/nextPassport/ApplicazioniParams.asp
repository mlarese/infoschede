...
'filtra per codice
if Session("sid_codice")<>"" then
	sql = sql & IIF(sql = "", " WHERE ", " AND ") & " sid_codice LIKE '%" & Session("sid_codice") & "%' "
end if
						...
						<tr>
							<td>
								<table cellspacing="1" cellpadding="0" class="tabella_madre">
									...
									<tr><th colspan="2" <%= Search_Bg("sid_codice") %>>CODICE</th></tr>
									<tr>
										<td class="content" colspan="2">
											<input type="text" name="search_codice" value="<%= TextEncode(session("sid_codice")) %>" style="width:100%;">
										</td>
									</tr>
							...
							<tr>
								<td class="body">
									<table width="100%" border="0" cellspacing="1" cellpadding="0" align="left">
										...
										<% sql = "SELECT * FROM tb_siti INNER JOIN rel_siti_descrittori ON tb_siti.id_sito = rel_siti_descrittori.rsd_sito_id WHERE rsd_descrittore_id =" & rs("sid_id") 
										rsa.open sql, conn, adOpenStatic, adLockOptimistic, adCmdText
										if rsa.eof then %>
											<tr>
												<td class="label">applicativi</td>
												<td class="content alert" colspan="3">Nssun applicativo associato.</td>
											</tr>
										<% else %>
											<tr>
												<td class="label" rowspan="<%= rsa.recordcount %>">applicativi</td>
												<%while not rsa.eof
													if rsa.absoluteposition > 1 then%>
														<tr>
													<% end if %>
													<td class="content" colspan="3"><%= rsa("sito_nome") %></td>
													</tr>
													<% rsa.movenext
												wend %>
										<% end if
