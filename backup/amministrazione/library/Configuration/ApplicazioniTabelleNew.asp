<%@ Language=VBScript CODEPAGE=65001%>
<% Option Explicit %>
<% response.charset = "UTF-8" %>
<!--#INCLUDE FILE="../Tools4Color.asp" -->
<!--#INCLUDE FILE="../database/Tools4Database.asp" -->
<!--#INCLUDE FILE="../../nextPassport/ToolsApplicazioni.asp" -->
<%
'verifica dei permessi
CALL VerificaPermessiUtente(true)

if Request.ServerVariables("REQUEST_METHOD")="POST" then
	Server.Execute("ApplicazioniTabelleSalva.asp")
end if

dim i, conn, rs, sql
set conn = Server.CreateObject("ADODB.Connection")
conn.open GetConfigurationConnectionstring()
set rs = Server.CreateObject("ADODB.Recordset")
%>

<%'--------------------------------------------------------
sezione_testata = "inserimento nuova tabella" %>
<!--#INCLUDE FILE="../Intestazione_Ridotta.asp" -->
<%'----------------------------------------------------- %>

<div id="content_ridotto">
	<form action="" method="post" id="form1" name="form1">
		<input type="hidden" name="tfn_tab_sito_id" value="<%= request("SITO_ID") %>">
		<table cellspacing="1" cellpadding="0" class="tabella_madre">
			<caption>Inserimento nuova tabella</caption>
			<tr><th colspan="3">DATI DELLA TABELLA</th></tr>
			<tr>
				<td class="label_no_width" colspan="2">titolo</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_titolo" value="<%= request("tft_tab_titolo") %>" maxlength="255" style="width:50%;">
					(*)
				</td>
			</tr>
			<tr>
				<td class="label_no_width" colspan="2">tabella</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_name" value="<%= request("tft_tab_name") %>" maxlength="255" style="width:50%;">
					(*)
				</td>
			</tr>
			<tr>
				<td class="label_no_width" colspan="2">colore dei contentuti</td>
				<td class="content">
					<% CALL WriteColorPicker_Input("form1", "tft_tab_colore", request("tft_tab_colore"), "", true, false, "") %>
				</td>
			</tr>
			<tr>
				<td class="label_no_width" colspan="2">tipo di tabella</td>
				<td class="content">
					<select name="tfn_tab_db_tipo">
						<option value="0" <%= IIF(request.form("tfn_tab_db_tipo") = "0", "selected", "") %>>Tutti i DBMS</option>
						<option value="1" <%= IIF(request.form("tfn_tab_db_tipo") = "1", "selected", "") %>>Access</option>
						<option value="2" <%= IIF(request.form("tfn_tab_db_tipo") = "2", "selected", "") %>>SQL Server</option>
					</select>
				</td>
			</tr>
			<tr>
				<td class="label_no_width" colspan="2">sorgente tabella</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_from_sql" value="<%= request("tft_tab_from_sql") %>" maxlength="255" style="width:95%;">
					(*)
				</td>
			</tr>
			<tr><th colspan="3">CAMPI DI LETTURA DEI DATI PER L'INDICE DEI CONTENUTI</th></tr>
			<tr>
				<td class="label_no_width" colspan="2">chiave primaria</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_field_chiave" value="<%= request("tft_tab_field_chiave") %>" maxlength="255" style="width:95%;">
					(*)
				</td>
			</tr>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" colspan="2" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">codice</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0">
					<input type="text" class="text" name="tft_tab_field_codice_<%= LINGUE_CODICI(i) %>" value="<%= request("tft_tab_field_codice_"& LINGUE_CODICI(i)) %>" maxlength="255" style="width:89.8%;">
				</td>
			</tr>
			<%next %>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" colspan="2" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">titolo</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0">
					<input type="text" class="text" name="tft_tab_field_titolo_<%= LINGUE_CODICI(i) %>" value="<%= request("tft_tab_field_titolo_"& LINGUE_CODICI(i)) %>" maxlength="255" style="width:89.8%;">
					<% 	if LINGUE_CODICI(i) = LINGUA_ITALIANO then response.write "(*)" end if %>
				</td>
			</tr>
			<%next %>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" colspan="2" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">titolo alternativo</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0" style="vertical-align: top;">
					<input type="text" class="text" name="tft_tab_field_titolo_alt_<%= LINGUE_CODICI(i) %>" value="<%= request("tft_tab_field_titolo_alt_"& LINGUE_CODICI(i)) %>" maxlength="255" style="width:89.8%;">
				</td>
			</tr>
			<%next %>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" style="width:11%;" rowspan="<%= ubound(LINGUE_CODICI)+2 %>">gestione link</td>
					<td class="label_no_width" style="width:9%;" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">url</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0">
					<input type="text" class="text" name="tft_tab_field_url_<%= LINGUE_CODICI(i) %>" value="<%= request("tft_tab_field_url_"& LINGUE_CODICI(i)) %>" maxlength="255" style="width:89.8%;">
				</td>
			</tr>
			<%next %>
			<tr>
				<td class="label_no_width">parametro</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_parametro" value="<%= request("tft_tab_parametro") %>" maxlength="255" style="width:95%;">
				</td>
			</tr>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" colspan="2" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">descrizione</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0">
					<input type="text" class="text" name="tft_tab_field_descrizione_<%= LINGUE_CODICI(i) %>" value="<%= request("tft_tab_field_descrizione_"& LINGUE_CODICI(i)) %>" maxlength="255" style="width:89.8%;">
				</td>
			</tr>
			<%next %>
			<tr>
				<td class="label_no_width" rowspan="2">foto</td>
				<td class="label_no_width">thumbnail</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_field_foto_thumb" value="<%= request("tft_tab_field_foto_thumb") %>" maxlength="255" style="width:100%;">
				</td>
			</tr>
			<tr>
				<td class="label_no_width">zoom</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_field_foto_zoom" value="<%= request("tft_tab_field_foto_zoom") %>" maxlength="255" style="width:100%;">
				</td>
			</tr>
			<tr>
				<td class="label_no_width" colspan="2">visibilit&agrave;</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_field_visibile" value="<%= request("tft_tab_field_visibile") %>" maxlength="255" style="width:100%;">
				</td>
			</tr>
			<tr>
				<td class="label_no_width" colspan="2">ordine</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_field_ordine" value="<%= request("tft_tab_field_ordine") %>" maxlength="255" style="width:100%;">
				</td>
			</tr>
			<tr>
				<td class="label_no_width" rowspan="2">validit&agrave;</td>
				<td class="label_no_width">pubblicazione</td>
				<td class="content">
					<input type="text" class="text" name="tft_tab_field_data_pubblicazione" value="<%= request("tft_tab_field_data_pubblicazione") %>" maxlength="255" style="width:100%;">
				</td>
			</tr>
			<tr>
				<td class="label_no_width">scadenza</td>
				<td class="label_no_width">
					<input type="text" class="text" name="tft_tab_field_data_scadenza" value="<%= request("tft_tab_field_data_scadenza") %>" maxlength="255" style="width:100%;">
				</td>
			</tr>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" rowspan="<%= (ubound(LINGUE_CODICI)+1)*2 %>">meta tag</td>
					<td class="label_no_width" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">keywords</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0" style="vertical-align: top;">
					<textarea name="tft_tab_field_meta_keywords_<%= LINGUE_CODICI(i) %>" style="width:94.5%;" rows="2"><%= request("tft_tab_field_meta_keywords_"& LINGUE_CODICI(i)) %></textarea>
				</td>
			</tr>
			<%next %>
			<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label_no_width" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">description</td>
				<% end if %>
				<td class="content">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0" style="vertical-align: top;">
					<textarea name="tft_tab_field_meta_description_<%= LINGUE_CODICI(i) %>" style="width:94.5%;" rows="2"><%= request("tft_tab_field_meta_description_"& LINGUE_CODICI(i)) %></textarea>
				</td>
			</tr>
			<%next %>
			<tr>
				<td class="footer" colspan="3">
					(*) Campi obbligatori.
					<input style="width:23%;" type="submit" class="button" name="salva" value="SALVA">
					<input style="width:23%;" type="button" class="button" name="annulla" value="ANNULLA" onclick="window.close();">
				</td>
			</tr>
		</table>
	</form>
</div>
</body>
</html>
<%
set rs = nothing
%>
<script language="JavaScript" type="text/javascript">
	FitWindowSize(this);
</script>