...
{ 
  ...
  if (browserVersion > 0) 
    auxEv = "<a name='" + this.getID() + "' href='javascript:clickOnNode(\""+this.getID()+"\")'>" 
  else 
    auxEv = "<a>" 
...
{ 
  ...
  fullLink += " onClick=\"clickOnLink('"+this.getID()+"\', '"+this.link+"','"+this.target+"');return false;\" ";
  this.isRendered = 1
...
{ 
  ...
  if (!initializing && LINK_ONCLICKNODE)
  	document.location.href = "?ID" + folderObj.getID() + "&state=" + state + "#" + folderObj.getID()
}
...
var LINK_ONCLICKNODE = false // indica se il link è da richiamare anche sul click del +/-
