...
<%'--------------------------------------------------------
sezione_testata = "inserimento nuovo componente"
...
sql = "SELECT * FROM gv_articoli WHERE rel_id=" & cIntero(request("ARTID"))
rs.open sql, conn, adOpenForwardOnly, adLockReadOnly, adCmdText

sql = "SELECT art_id, art_se_bundle, art_se_confezione FROM gv_articoli WHERE rel_id=" & cIntero(request("EXT_ID"))
...
<div id="content_ridotto">
	<form action="" method="post" id="form1" name="form1">
		<input type="hidden" name="tfn_bun_bundle_id" value="<%= request("EXT_ID") %>">
		<input type="hidden" name="tfn_bun_articolo_id" value="<%= request("ARTID") %>">
		...
		<table cellspacing="1" cellpadding="0" class="tabella_madre">
			...
			<tr>
				...
				<td class="content" colspan="6">
					<input type="text" class="text" tabindex="1" name="tfn_bun_quantita" value="<%= request("tfn_bun_quantita") %>" maxlength="3" size="3"> (*)
				</td>
			</tr>
			<tr>
