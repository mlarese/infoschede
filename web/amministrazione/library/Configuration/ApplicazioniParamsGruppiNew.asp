...
<div id="content">
	...
	<input type="hidden" name="tfn_sdr_personalizzato" value="0">
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		...
		<%for i=lbound(LINGUE_CODICI) to ubound(LINGUE_CODICI)%>
			<tr>
				<% if i = 0 then %>
					<td class="label" rowspan="<%= ubound(LINGUE_CODICI)+1 %>">titolo:</td>
				<% end if %>
				<td class="content" colspan="3">
					<img src="../../grafica/flag_<%= LINGUE_CODICI(i) %>.jpg" width="26" height="15" alt="" border="0">
					<input type="text" class="text" name="tft_sdr_titolo_<%= LINGUE_CODICI(i) %>" value="<%= request.form("tft_sdr_titolo_"& LINGUE_CODICI(i)) %>" maxlength="250" size="75">
