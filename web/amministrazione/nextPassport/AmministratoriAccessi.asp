...
dicitura.sezione = "Gestione utenti area amministrativa - log accessi"
if request("ID")="" then
	dicitura.puls_new = ""
	dicitura.link_new = ""
else
...
sql = "SELECT * FROM (log_admin INNER JOIN tb_admin ON log_admin.log_admin_id = tb_admin.id_admin) " &_
  	  " INNER JOIN tb_siti ON log_admin.log_sito_id = tb_siti.id_sito " 
if request("ID")<>"" then
	sql = sql & " WHERE id_admin=" & cIntero(request("ID"))
end if
