...
sub write_permessi(conn, rs, amministrazione, utente, permessi_per_riga)
	dim sql, rsP, checked, i, est, estAbilita, estNome, conta, max_num_permessi, lim_inf, lim_sup, num_col, fine, addHidden
								...
								if (rs("id_sito") <> cInteger(Session("ID_SITO"))) OR i>1 OR _	
									...
									if utente>0 then
										if amministrazione then
										 ...
										 rsp.open sql, conn, adOpenForwardOnly, adLockReadOnly, adAsyncFetch
										 if not rsp.eof then
									...
									<% if cString(Session("PASS_ADMIN"))="" AND i=1 AND rs("sito_protetto") AND addHidden then %>
										<input type="hidden" name="chk_perm" id="chk_<%= rs("id_sito") %>" value="<%=rs("id_sito")%>,<%=i%>" <%= checked %>>
									<% end if %>	
									<input <%=IIF(cString(Session("PASS_ADMIN"))="" AND i=1 AND rs("sito_protetto"),"disabled","")%> type="checkbox" name="chk_perm" id="chk_<%= rs("id_sito") %>" value="<%=rs("id_sito")%>,<%=i%>" <%= checked %>>
									&nbsp;<%=rs("sito_p" & i)%>
								<%end if %>
							</td>
						<%else
...
'.................................................................................................................
'procedura che scrive nel log le modifiche subite dagli utenti e che invia l'email di avviso all'amministratore.
'conn:				connessione aperta al database
'codOperazione:		contiene uno dei seguenti valori: 		AdminPassword 		(modifica della password)
'															AdminInserimento	(inserimento nuovo utente)
'															AdminModifica		(modifica dati utente esitente)
'admin_id:			id dell'amministratore modificato
'admin_login:		login dell'amministratore modificato
'admin_email:		email dell'amministratore modificato
'.................................................................................................................
sub LogModificaUtente(conn, codOperazione, admin_id, admin_login, admin_email)
	dim subject, text
	
	Select case Ucase(codOperazione)
		case "ADMINPASSWORD"
			subject = "Modifica password utente area amministrativa"
		case "ADMININSERIMENTO"
			subject = "Inserimento nuovo utente area amministrativa"
		case else
			subject = "Modifica dati utente area amministrativa"
	end select
	
	text = "ID : " & admin_id & vbcrlf + _
		   "Login : " & admin_login & vbcrlf & _
		   "e-mail : " & admin_email & vbcrlf
		   
	if Ucase(codOperazione) = "ADMININSERIMENTO" then
		text = "Utente inserito:" + vbCrLF + _
			   text + vbCrLf + _
			   "Inserito da:" + vbCrLF
	else
		text = "Utente modificato:" + vbCrLF + _
			   text + vbCrLf + _
			   "modificato da:" + vbCrLF
	end if

	text = text + _
		   "ID : " & Session("ID_ADMIN") & vbcrlf + _
		   "Login : " & Session("LOGIN_4_LOG") & vbcrlf & _
		   "Data : " & DateTimeIta(Now())
	
	'scrive nel log la modifica
	CALL WriteLogAdmin(conn, "tb_admin", admin_id, codOperazione, ParseSQL(text, adChar))
	
	'invio e-mail all'amministratore (sviluppo@combinario.com)
	CALL SendEmailSupportEX(Request.ServerVariables("SERVER_NAME") + " - " + subject, text) 
end sub
