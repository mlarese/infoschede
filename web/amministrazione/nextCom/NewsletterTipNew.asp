...
Titolo_sezione = "Tipologie Newsletter - inserimento"
	...
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		...
		<tr>
			...
			<td class="content" colspan="3">
				<!--<img src="../grafica/flag_it.jpg" width="26" height="15" alt="" border="0">-->
				<input type="text" class="text" name="tft_nl_nome_it" value="<%= request("tft_nl_nome_it")%>" maxlength="250" size="79">
				(*)
			</td>
		</tr>
		<tr>
			...
			<td class="content" colspan="2">
				<% CALL DropDownPages(conn, "form1", "410", 0, "tfn_nl_pagina_id", request("tfn_nl_pagina_id"), true, false) %>
		...
		<tr>
			...
			<td class="content">
				<input type="radio" class="checkbox" value="1" name="tfn_nl_gestione_dinamica_contenuti" <%= chk(cInteger(request("tfn_nl_gestione_dinamica_contenuti"))>0) %>>
				si
				<input type="radio" class="checkbox" value="0" name="tfn_nl_gestione_dinamica_contenuti" <%= chk(cInteger(request("tfn_nl_gestione_dinamica_contenuti"))=0) %>>
				no
			</td>
		</tr>
		<tr>
			<td class="label_no_width" style="width:17%;">lingua:</td>
			<td class="content" colspan="2"><% CALL DropLingue(conn, rs, "tft_nl_lingua", request("tft_nl_lingua"), true, false, "") %></td>
		</tr>
		<tr><th colspan="4">Scegli i destinatari di default</th></tr>
		<tr>
			<td class="label">rubriche:</td>
			<input type="hidden" name="tft_nl_rubriche_default" value="<%= request("tft_nl_rubriche_default") %>">
				...
				<table border="0" cellspacing="0" cellpadding="0" style="width:100%">
					<tr>
						<td style="width:95%;" colspan="2">
							<% 
							dim messageType, rubricheNameList, contattiIdList, sql
							contattiIdList = request("tft_nl_contatti_default")
							messageType = MSG_EMAIL
							
							if cString(request("tft_nl_rubriche_default")) = "" then
								sql = "0"
							else
								sql = Replace(request("tft_nl_rubriche_default"), ";", ",")
								sql = Trim(sql)
