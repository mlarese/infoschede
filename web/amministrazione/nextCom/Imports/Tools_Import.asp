...
<table cellspacing="1" cellpadding="0" class="tabella_madre" style="border-bottom:0px;">
    <tr>
    	...
    	<td class="content" colspan="3">
    		...
    		CALL dropDown(conn, sql, "id_rubrica", "nome_rubrica", "rubrica_import", request("rubrica_import"), false, "", LINGUA_ITALIANO)
			%>(*)<br>
...
'funzione che restituisce il valore del canmpo sorgente solo se esiste nella sorgente.
function SourceField(rs, FieldName, isString)
	dim campo
	if FieldExists(rs, FieldName) then
'response.write "-" & FieldName & "-<br>"
		campo = cString(rs(FieldName))
		if isString AND not IsNull(rs(FieldName)) then
