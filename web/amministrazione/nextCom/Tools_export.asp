...
sub Export_Excel2000(conn, rs, sql, rsv)
	dim FileName, FSO, ExportPath, XLSConnString, XLSConn, XLSrs, Xsql, field, FileStream, rsN, rsR, value
	...
	'preparo la tabella tb_valoriNumeri
	sql = "SELECT ValoreNumero, id_Indirizzario, id_TipoNumero, protetto_privacy FROM tb_ValoriNumeri "
	rsN.open sql, conn, adOpenStatic, adLockReadOnly, adCmdText
	Set rsN.ActiveConnection = Nothing
	
	'preparo la tabella tb_rubriche
	sql = " SELECT nome_rubrica, id_indirizzo FROM tb_rubriche " & _
		  " INNER JOIN rel_rub_ind ON tb_rubriche.id_rubrica = rel_rub_ind.id_rubrica "
	rsR.open sql, conn, adOpenStatic, adLockReadOnly, adCmdText
	Set rsR.ActiveConnection = Nothing	
	...
	while not rs.eof
		...
		while not rsv.eof
			...
			'sql = "SELECT ValoreNumero FROM tb_ValoriNumeri " &_
		 	'	  " WHERE NOT " & SQL_IsTrue(conn, "protetto_privacy") & " AND id_TipoNumero=" & rsv("id_tipoNumero") & " AND  id_Indirizzario=" & rs("ID")
			'XLSrs(field) = GetValueList(conn, rsN, sql)
			rsN.Filter = adFilterNone
			rsN.Filter = " id_Indirizzario=" & rs("ID") & " AND id_TipoNumero=" & rsv("id_tipoNumero") & " AND protetto_privacy = 0"
			value = ""
			while not rsN.eof
				value = value & rsN("ValoreNumero") & ", "
				rsN.moveNext
			wend
			if value <> "" then
				XLSrs(field) = Left(value, Len(value) - 2)
			end if
		...
		rsR.Filter = adFilterNone
		rsR.Filter = " id_indirizzo = " & rs("ID")
		value = ""
		while not rsR.eof
			value = value & rsR("nome_rubrica") & ", "
			rsR.moveNext
		wend
		if value <> "" then
			XLSrs("Rubriche") = Left(value, Len(value) - 2)
		end if
