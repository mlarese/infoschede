...
dicitura.sezione = "Gruppi controlli per riparazioni - modifica"
dicitura.puls_new = "INDIETRO"
dicitura.link_new = "DescrRag.asp"
dicitura.scrivi_con_sottosez() 

sql = "SELECT * FROM sgtb_descrittori_raggruppamenti WHERE rag_ID=" & cIntero(request("ID"))
...
<div id="content">
	...
	<table cellspacing="1" cellpadding="0" class="tabella_madre">
		<caption>Modifica dati gruppo di caratteristiche delle anagrafiche</caption>
		...
		<%for i=lbound(Application("LINGUE")) to ubound(Application("LINGUE"))%>
			<tr>
				...
				<td class="content" colspan="3">
					...
					<input type="text" <%= disabled %> class="text" name="tft_rag_titolo_<%= Application("LINGUE")(i) %>" value="<%= rs("rag_titolo_"& Application("LINGUE")(i)) %>" maxlength="250" size="75">
				<% 	if Application("LINGUE")(i) = LINGUA_ITALIANO then
						response.write "(*)"
						if disabled <> "" then %>
					<input type="hidden" name="tft_rag_titolo_it" value="<%= rs("rag_titolo_it") %>" maxlength="250" size="75">
				<%		end if
					end if %>
				</td>
			</tr>
		...
		<tr>
			...
			<td class="content" width="35%">
				<input type="text" <%= disabled %> class="text" name="tfn_rag_ordine" size="3" value="<%= rs("rag_ordine") %>">
			</td>
			<td class="label">codice:</td>
			<td class="content">
				<input type="text" <%= disabled %> class="text" name="tft_rag_codice" value="<%= rs("rag_codice") %>" maxlength="50" >
			</td>
		</tr>
		...
		<tr>
			...
			<% 	'seleziono solo descrittori associati a raggruppamenti non locked
				sql = " SELECT *,"& _
					  ...
					  " ("& SQL_If(conn, "des_raggruppamento_id = "& cIntero(request("ID")), "1", "NULL") &") AS rel"& _
				...
				CALL Write_Relations_Checker(conn, rsc, sql, 2, "des_id", "nome", "rel", "car") %>
			</td>
		</tr>
		<tr><th colspan="4">NOTE INTERNE</th></tr>
		<tr>
			<td class="content" colspan="4">
				<textarea style="width:100%;" rows="3" name="tft_rag_note"><%=rs("rag_note")%></textarea>
			</td>
		</tr>
		<tr>
