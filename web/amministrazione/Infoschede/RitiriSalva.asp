...
dim Classe
	...
	Classe.Requested_Fields_List	= "tfn_ddt_cliente_id;tfn_ddt_destinazione_id;tfn_ddt_trasportatore_id"
...
Sub Gestione_Relazioni_record(conn, rs, ID)
	...
	'inserissco il numero del ritiro, se è un nuovo inserimento
	...
	sql = "UPDATE sgtb_schede SET sc_costo_presa = 0 WHERE ISNULL(sc_documento_ritiro_id, 0)=0 AND sc_cliente_id = " & cIntero(request("tfn_ddt_cliente_id"))
	CALL conn.execute(sql, , adExecuteNoRecords)
	...
	if Trim(Replace(request("id_schede"),",",""))<>"" then
		...
		for each ids in listaId
			...
			sql = "UPDATE sgtb_schede SET sc_documento_ritiro_id = " & ID & " WHERE sc_id = " & ids
			conn.Execute(sql)
			if cString(request("costo_ritiro_scheda_"&ids))<>"" then
				sql = "UPDATE sgtb_schede SET sc_costo_presa = " & ParseSQL(request("costo_ritiro_scheda_"&ids), adNumeric) & " WHERE sc_id = " & ids
				conn.Execute(sql)
			end if
		next
	else
	...
	if request("reload")="true" then
		...
		if request.form("salva") <> "" then
			Classe.Next_Page = "RitiriMod.asp?ID="& ID
		end if
	end if
