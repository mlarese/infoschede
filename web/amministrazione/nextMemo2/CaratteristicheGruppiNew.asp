...
dicitura.sezione = "Gruppi di caratteristiche - nuovo"
		...
		<tr>
			...
			<td class="content" colspan="3">
				<input type="text" class="text" name="tfn_ctr_ordine" size="3"
					   value="<%= IIF(request.servervariables("REQUEST_METHOD")<>"POST", CIntero(GetValueList(conn, NULL, "SELECT MAX(ctr_ordine) FROM mtb_carattech_raggruppamenti")) + 1, request.form("tfn_ctr_ordine")) %>">
			</td>
		</tr>
		<tr>
