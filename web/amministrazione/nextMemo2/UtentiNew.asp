...
if request.form("salva")<>"" then
	dim cnt_id, ut_id, mandatory_flds, val
	...
	if OBJ_contatto.ValidateLoginAndPassword(request("old_login"), request("conferma_password")) then
		...
		if OBJ_contatto.ValidateFields(mandatory_flds, true)	then
			...
			if Session("PERMESSO_AREA_RISERVATA") <> "" then
				ut_id = OBJ_Contatto.UserFromContact(cnt_id, Session("PERMESSO_AREA_RISERVATA"))
			else
				ut_id = OBJ_Contatto.UserFromContact(cnt_id, 0)
			end if

			
			'inserimento relazioni tra utente e profili
			...
			<tr>
				<td class="content" colspan="5">
					<% sql = " SELECT *, NULL AS valore FROM mtb_profili " + _
					   ...
					   CALL Write_Relations_Checker(OBJ_contatto.conn, rs, sql, 4, "pro_id", "pro_nome_it", "valore", "profili_associati")%>
				</td>
			</tr>
		<% end if %>
		<tr>
